...
@csrf_exempt
def get_point_cloud_app(request):
    try:
        json_bytes = request.body
        circle_point_dict = json.loads(json_bytes)
        # id = request.POST.get["id"]
        id = circle_point_dict['id']
        if CONFIG_FILE.CLOUD_ID > id:
            message_info = {
                "message": False,
                "cause": '点云瓦片数据不存在'
            }
        ...
        if id == 0:
            ...
            circle_list = cache.get('CIRCLE_DATA')
            track_list_temporary = cache.get('TRACT_DATA')
            point_list_temporary = cache.get('point_cloud')

            if point_list_temporary is not None:
                point_list = point_list_temporary[id:]
            if track_list_temporary is not None:
                track_list = track_list_temporary[id:]
                track_list = track_list[:len(point_list)]
        if point_list:  # 需要返回点云 1 0 3
            CONFIG_FILE.CLOUD_ID = id + 1
            if circle_list is None:
                circle_list = []
            if track_list is None:
                track_list = []
        ...
        return JsonResponse('err', status=202, safe=False)
...
#  获取排序project列表,优化json 对下载文件item进行排序 get_project_list
@csrf_exempt
...
def disk(request):
    ...
    return JsonResponse({'message': 'OK', 'all_space': all_space, 'space': space}, status=200, safe=False)
